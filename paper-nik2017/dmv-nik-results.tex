...
Mercurial commits are atomic, and we saw that in each case, the commit was not stored, and the repository was left unchanged.
Mercurial needs to be able to fit the file into memory three times over in order to commit it.
\perotto{Give reason Mercurial needs 3x RAM}
\askotto{Should that go in the discussion section?}
...
\perotto{Make clearer that this was just experiment set up and can be changed}
...
\perotto{Why parabolic}
\askotto{In discussion?}
...
\perotto{Can be configured with more inodes (max in Linux is...?)}
