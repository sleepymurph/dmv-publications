...
\subsection{Autonomous nodes and free-form architecture}
...
\paragraph{Availability}
  Replicas are autonomous and always available to themselves.

\paragraph{Explicit inconsistency}
  Synchronization is explicit and manual. States are specifically allowed to
  diverge. Each replica manually decides what updates to incorporate from other
  sources.

\paragraph{Scaling}
  Local autonomy of replicas and explicit inconsistency allows networks of
  replicas to mirror the connections of their users.

  Many small projects use a hub-and-spoke topology, designating one replica as
  the main replica, and others pull from it and push to it.
...
Possible mitigation: virtual working directory (e.g. via FUSE) as a
