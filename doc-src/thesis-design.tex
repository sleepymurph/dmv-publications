...
\gls{DMV}'s data set and its history are represented as a DAG, and the rest of
the design flows from that. \gls{DMV}'s DAG is based on Git's, but it adds a new
object type, the \gls{chunkedblob}, which represents a \gls{blob} that has been
broken into several smaller chunks. An example \gls{DMV} DAG is shown in
...
The object types that make up the \gls{DMV} DAG are as follows:
...
to a partial DAG. A node that stores only part of the data set can create new
