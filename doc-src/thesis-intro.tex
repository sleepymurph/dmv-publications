...
\newterm{tree} object that maps filenames to SHA-1 \gls{blob} IDs for each file
in the directory. This tree object is also stored in the database with it's
SHA-1 hash as its ID. Tree objects can refer not only to \gls{blob}, but to
other trees, representing subdirectories.

Thus, a file hierarchy in a given state is represented by a hash tree, with
\newterm{tree} objects as nodes and \glspl{blob} as leaves, and the entire
state can be referred to by a single hash ID, that of the top-level tree object.
A simple example is shown in \autoref{dia_git_dag_example_simple_tree}.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\textwidth]{dia_git_dag_example_simple_tree}
    \caption{A simple file hierarchy represented by Git \newterm{tree} and
    blob objects and their SHA-1 hash IDs}
    \label{dia_git_dag_example_simple_tree}
\end{figure}

Git then links different file hierarchy states with \newterm{commit} objects. A
commit object includes a hash ID for a tree, representing the state of the file
hierarchy, and one or more hash IDs of parent commits, representing the previous
states that this one was built from. Like with \glspl{blob} and trees, the commit
object is also hashed and stored with the hash as its ID.

The resulting data structure is a directed graph, with new commits pointing to
...
resulting \glspl{blob} and trees will be identical in content and thus have the same
hash ID. The directed acyclic graph is referred to as the \newterm{DAG}. It can
be uniquely identified by the hash IDs of those commits which do not yet have
child commits to refer to them, the \newterm{heads} of each current branch of
...
A Git repository, then, is a collection of the \gls{blob}, tree, and commit objects
that make up the file set's history, stored by hash ID in the object store, with
...
The main weakness of Git's DAG is that \glspl{blob} and files are one and the same.
