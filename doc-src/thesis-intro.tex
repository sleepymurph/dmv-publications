...
One of the key aspects of Git is that data is stored according to its content;
it is \newterm{content-addressable}. A file is stored by prepending a short Git
header and then taking the SHA-1 hash of the resulting \newterm{blob} (binary
...
\newterm{tree}, and its binary representation is also hashed by SHA-1 to get a
