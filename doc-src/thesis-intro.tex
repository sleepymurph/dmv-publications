...
When Git stores a file, it creates a \newterm{blob} (Binary Large OBject) by
copying the file's content and prepending a short header. Git then calculates
the SHA-1 hash of the blob, and stores the blob in an \newterm{object store},
using the SHA-1 hash as the blob's ID. To store a directory, Git creates a
\newterm{tree} object that maps filenames to SHA-1 blob IDs for each file in the
directory. This tree object is also stored in the database with it's SHA-1 hash
as its ID. Tree objects can refer not only to blobs, but to other trees,
representing subdirectories.

Thus, a file hierarchy in a given state is represented by a hash tree, with
\newterm{tree} objects as nodes and \newterm{blobs} as leaves, and the entire
...
states that this one was built from. Like with blobs and trees, the commit
