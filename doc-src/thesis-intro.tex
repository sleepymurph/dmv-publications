...
source code: plain text files in the range of tens of kilobytes. Checking in
larger binary files such as images, sound, or video affect performance. Actions
that require copying data in and out of the system slow from hundredths of a
second to full seconds or minutes. And since a \gls{DVCS} keeps every version of
every file in every repository, forever, the disk space needs compound.
...
\blockcquote[p.33]{continuousdeliverybook}{
    ...
    \askotto{Is this quote maybe off topic/overkill?}
}
...
and Durability\cite[Chapter 1]{nosqldistilled}. As demand increased, the server
would be scaled up, beefing up the server hardware with more disk space, more
RAM, and more and faster CPUs\cite[Chapter 4]{nosqldistilled}. But there is a
limit to scaling up the hardware of a single machine, and as data kept growing,
a new wave of systems appeared that scaled \emph{out} to many commodity servers,
distributed systems\cite[Chapter 4]{nosqldistilled}.
...
files\cite{history_of_version_control,svnbook}. Doing so made branching easier,
and branching quickly became a key feature for collaborative work.
...
use\cite{what_are_devs_talking_about}, making \glsdisp{DVCS}{distributed version
control} the dominant paradigm for \glsdisp{SCM}{source code management}.
...
\begin{figure}[]
    \centering
        \includegraphics[width=0.5\textwidth]{dia_git_dag_types}
