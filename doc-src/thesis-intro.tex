...
storage}. A file is stored by\perotto{"no"}\askotto{"No"? Am I wrong?}
prepending a short Git header and then taking the SHA-1 hash of the resulting
\newterm{blob} (binary large object). That SHA-1 hash becomes the blob's ID. A
directory is stored by creating a map of file names in the directory to each
file's SHA-1 blob ID. This mapping is a called a \newterm{tree} object, and its
...
\begin{description}
    ...
    \item[Atomicity] Commits are atomic, since all objects that they refer to
        have to be added first, then the commit object\perotto{yes, what about
        it?}, then finally the ref is updated to point to the new commit. The
...
scenes. \perotto{new para}But this is an optimization. There can also be trouble
when a very large file becomes a large object, breaking
assumptions\perotto{Explain "trouble", what assumptions are broken, and what
consequences are} that algorithms might have about fitting objects into memory
...
media versioning system\perotto{Make this more visible: new paragraph, italics,
something}.
