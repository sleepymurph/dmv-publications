...
\perottoinline{"Performance" vague. Be more specific or sum up what performance
you're talking about here at beginning of chapter.}
...
than \SI{1}{\ms}. The others are plotted in \autoref{plot-seek-times}, and data
about the top ten longest writes is listed in \autoref{longest-writes}. The
spikes in the graph are arranged in curves radiating out from zero files and
zero directories. Each curve represents a cluster of directory schemes that
filled up the disk in the same pattern, corresponding more to subdirectory depth
than to number of hex digits per subdirectory.
...
Those are the only two writes that took longer that \SI{2}{\second}. Five took
