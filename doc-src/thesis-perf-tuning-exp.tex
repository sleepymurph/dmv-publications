...
So, only very low values for the number of hex digits (\num{3} or less) and
subdirectory depth (\num{3} or less) are acceptable.

From there, we turn to the mysterious, intermittent long write times. In our
experiment, \num{312813} of \num{315601} writes (\SI{99.1}{\percent}) of writes
completed in less than \SI{1}{\ms}. The others are plotted in
\autoref{plot-seek-times}. The spikes are as high as \SI{2}{\s}, though most
seem to be under \SI{1}{\s}. There is no clear pattern to their occurrence in
terms of number of files or directories, though they tend to get longer as both
increase. They are spaced apart somewhat regularly. It would seem they are
caused by upkeep that the filesystem has to do periodically, and that there is
no obvious way to avoid them, at least not while storing many small files.
