...
in all of the version-control systems, such as hashing the data and, in the case
of Git and Mercurial, calculating deltas.
...
than the \lstinline{/proc/stat} ticks used to CPU measure activity. At such
small sizes, most measurements will yield zero ticks, though one of the states
will occasionally measure 1 tick (see \autoref{copy-cpu-data}). Then, when the
graph normalizes the total usage to a percentage, \num{1} system tick out of
\num{1} total ticks makes \SI{100}{\percent}. It isn't until around
\SI{48}{\mib} that the measurement includes enough CPU ticks to yield useful
percentages, which is where we see the graph start to stabilize.
...
a fixed number of them\todo{cite}. Each of these programs creates one file in
...
Both Git and Mercurial converge to a little over 8x the space required. This
probably has more to do with the filesystem block size than anything else. The
underlying file system uses a 4KiB block size, so each 1KiB file will still use
4KiB of disk space. And since there are two copies of each file, that's 8KiB
total for each 1KiB file, 8x the disk space.

Mercurial converges towards the 8x limit faster though. We speculate this is
because of lower repo overhead, and also because Git is creating \gls{tree}
objects for each of the subdirectories in the file set. These files will be
small, but each will take up another 4KiB block on the disk.
