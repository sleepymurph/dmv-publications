...
the \gls{commit} would not be stored. However, the \gls{repository} would be
left in a consistent empty state. The atomicity of the \gls{commit} operation
held.
...
no consistency errors in the \gls{repository} reported by \lstinline{git fsck}.
...
report any actual integrity errors in the \gls{repository}.
...
fsck command again with the corrupted \gls{repository}, it reported the
