...
Applications built on DMV may add daemons to automatically \gls{commit} and
sync, but DMV focuses only on providing the commands.

DMV is used from the command line and includes familiar subcommands such as
branch, \gls{commit}, and checkout
...
porting DMV to low-powered or mobile devices\perotto{"(!)"}\askotto{"?"}, and it allows us to compile DMV as
a library.
