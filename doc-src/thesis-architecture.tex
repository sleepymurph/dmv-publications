...
Each \gls{DMV} node is a \gls{repository} consisting of a content-addressable
\gls{objectstore} for immutable \gls{DAG} objects and a \gls{workdir} for active
file editing (\autoref{fig:dia_obj_db_and_wd}). \Glspl{repository} that are used
only for storage can omit the \gls{workdir}, similar to a Git bare
\gls{repository}.
...
\perottoinline{Can the functionality (course-grained) be defined here?}
